...
from transformers_expand.data import DataCollatorForTokenClassificationWithGlobalPointer
...
class ModelArguments:
    ...
    inner_dim: int = field(
        default=None,
        metadata={"help": "(Efficient)GlobalPointer inner_dim size. default = 64."},
    )
    use_efficient: bool = field(
        default=False,
        metadata={"help": "Will use EfficientGlobalPointer layer instead of GlobalPointer layer."}
    )
...
def main():
    ...
    model = AutoModelForTokenClassificationWithGlobalPointer.from_pretrained(
        ...
        inner_dim=64 if model_args.inner_dim is None else model_args.inner_dim,
        use_efficient=model_args.use_efficient
    )
    ...
    # Data collator
    data_collator = DataCollatorForTokenClassificationWithGlobalPointer(tokenizer,
                                                                        num_labels=num_labels,
                                                                        max_length=data_args.max_seq_length,
