# -*- coding: utf-8 -*-
# @Time     : 2022/11/27 21:05
# @File     : modeling_mobilebert.py.py
# @Author   : Zhou Hang
...
@add_start_docstrings(
    """
    MobileBert Model with a token classification head on top (a biaffine layer on top of the hidden-states output) 
    e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    MOBILEBERT_START_DOCSTRING,
)
# Copied from transformers.models.bert.modeling_bert.BertForTokenClassification with Bert->MobileBert all-casing
class MobileBertForTokenClassificationWithBiaffine(MobileBertPreTrainedModel):
...
@add_start_docstrings(
    """
    MobileBert Model with a token classification head on top (a global pointer layer on top of the hidden-states output) 
    e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    MOBILEBERT_START_DOCSTRING,
)
# Copied from transformers.models.bert.modeling_bert.BertForTokenClassification with Bert->MobileBert all-casing
class MobileBertForTokenClassificationWithGlobalPointer(MobileBertPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
