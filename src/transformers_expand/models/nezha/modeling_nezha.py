...
@add_start_docstrings(
    """
    Nezha Model with a token classification head on top (a biaffine layer on top of the hidden-states output) 
...
@add_start_docstrings(
    """
    Nezha Model with a token classification head on top (a global pointer layer on top of the hidden-states output) 
    e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    NEZHA_START_DOCSTRING,
)
class NezhaForTokenClassificationWithGlobalPointer(NezhaPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
