...
class XLMRobertaXLForTokenClassificationWithBiaffine(XLMRobertaXLPreTrainedModel):
...
@add_start_docstrings(
    """
    XLM-Roberta-xlarge Model with a token classification head on top (a global pointer layer on top of the hidden-states output) 
    e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    XLM_ROBERTA_XL_START_DOCSTRING,
)
class XLMRobertaXLForTokenClassificationWithGlobalPointer(XLMRobertaXLPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
