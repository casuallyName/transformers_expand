# -*- coding: utf-8 -*-
# @Time     : 2022/11/27 11:11
# @File     : modeling_esm.py.py
# @Author   : Zhou Hang
...
@add_start_docstrings(
    """
    ESM Model with a token classification head on top (a biaffine layer on top of the hidden-states output) 
    e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    ESM_START_DOCSTRING,
)
class EsmForTokenClassificationWithBiaffine(EsmPreTrainedModel):
...
@add_start_docstrings(
    """
    ESM Model with a token classification head on top (a global pointer layer on top of the hidden-states output) 
    e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    ESM_START_DOCSTRING,
)
class EsmForTokenClassificationWithGlobalPointer(EsmPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
