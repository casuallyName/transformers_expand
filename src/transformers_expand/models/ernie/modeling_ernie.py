...
class ErnieForTokenClassificationWithBiaffine(ErniePreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]

    def __init__(self, config, biaffine_input_size: int = None, use_lstm: bool = None):
        super().__init__(config)
        # 此处 +1 操作是由于实体标签类别中含有一个 "非实体" 类别，即label map中的 O
...
class ErnieForTokenClassificationWithGlobalPointer(ErniePreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]

    def __init__(self, config, inner_dim: int = 64, use_efficient: bool = False):
        super().__init__(config)
